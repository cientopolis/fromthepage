class Api::TranscribeController  < Api::ApiController
  ...
  before_action :set_data, :only => [:save_transcription, :save_translation]
  ...
  def save_transcription
    ...
    if params['save']
      ...
      begin
        if @page.save
          ...
          if current_user.guest?
            ...
            if deeds < 3
              ...
              render_serialized ResponseWS.ok('api.transcription.save.success_guest',@page);
            else
              ...
              render_serialized ResponseWS.simple_error('api.transcription.save.error.guest_limit');
            end
          else
        ...
        logger.fatal "\n\n#{ex.class} (#{ex.message}):\n"
        render_serialized ResponseWS.simple_error('api.transcription.save.error.parse')
        # render :action => 'display_page'
        # raise ex
      rescue  => ex
        log_transcript_exception(ex, message)
      ...
      begin
        if @page.save
          ...
          if current_user.guest?
            deeds = Deed.where(user_id: current_user.id).count
            if deeds < 3
              render_serialized ResponseWS.ok('api.translation.save.success_guest',@page)
            else
              # session[:user_return_to]=collection_translate_page_path(@collection.owner, @collection, @work, @page.id)
              # redirect_to new_user_registration_path, :resource => current_user
              
              render_serialized ResponseWS.simple_error('api.translation.save.error.guest_limit');
            end
          end
          
          # redirect_to :action => 'assign_categories', page_id: @page.id, collection_id: @collection, :translation => true
          render_serialized ResponseWS.ok('api.translation.save.success',@page)
        else
        ...
        # flash.clear
        # raise ex
        render_serialized ResponseWS.simple_error('api.translation.save.error.parse')
      rescue  => ex
        log_translation_exception(ex, message)
        # flash[:error] = ex.message
        logger.fatal "\n\n#{ex.class} (#{ex.message}):\n"
